...
public class Example04 {
	...
	public static void main(String[] args) {
		...
		List<FazendaVO> fazendasFiltradas = FluentIterable.from(fazendas).filter(Predicates.and(FILTER_FAZENDAS_MAIOR_IGUAL_7_TALHOES, FILTER_FAZENDAS_MENOR_IGUAL_10_EQUIPAMENTOS)).toList();

		System.out.println(fazendasFiltradas.size());
		System.out.println(fazendasFiltradas);

		fazendasFiltradas = FluentIterable.from(fazendas).filter(Predicates.or(FILTER_FAZENDAS_MAIOR_IGUAL_7_TALHOES, FILTER_FAZENDAS_MENOR_IGUAL_10_EQUIPAMENTOS)).toList();
